...
export async function createBookingHandler(c: Context): Promise<Response> {
  ...
  const requiresStoredCard = !hasActiveSubscription || isEmergency

  if (requiresStoredCard && !defaultCard) {
    throw new AppError({
      statusCode: 402 as any,
      message:
        'Payment method required: add a default card before creating PAYG or emergency bookings.',
  ...
  const emergencyAuthCardLast4 =
    isEmergency && defaultCard ? defaultCard.last4 : null
  ...
  const booking = await db.$transaction(async (tx) => {
    ...
    // Notify operator
    const feeDisplay = emergencyFeeAmount
      ? `£${emergencyFeeAmount.toFixed(2)}`
      : '£150.00'
