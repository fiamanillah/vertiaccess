...
export async function chargeEmergencyBooking(params: {
    ...
    const booking = await db.booking.findUnique({
        where: { id: bookingId },
        include: {
            site: { select: { id: true, name: true, landownerId: true, status: true } },
            operator: {
                include: {
...
export async function chargeEmergencyHandler(c: Context): Promise<Response> {
    ...
    const trigger = body?.trigger === 'admin_dispute' ? 'admin_dispute' : 'operator_confirmed';
...
export async function adminDisputeChargeHandler(c: Context): Promise<Response> {
    ...
    if (booking.useCategory !== 'emergency_recovery') {
        throw new AppError({
            ...
            message: 'Admin dispute charge only applies to Emergency & Recovery bookings',
            code: 'BAD_REQUEST',
        });
    }
    ...
    const result = await chargeEmergencyBooking({ bookingId, trigger: 'admin_dispute' });
 ...
 */
export async function retryOverduePaymentHandler(c: Context): Promise<Response> {
    const cognitoUser = c.get('cognitoUser') as CognitoUser;
