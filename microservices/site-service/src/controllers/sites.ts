...
export async function createSiteHandler(c: Context): Promise<Response> {
  ...
  await recordAuditLog(undefined, {
    ...
    actorType: (cognitoUser.role || 'assetmanager').toLowerCase() as any,
    actorId: effectiveUserId,
    newState: { status: 'UNDER_REVIEW', name: site.name },
  })
...
export async function updateSiteHandler(c: Context): Promise<Response> {
  ...
  await recordAuditLog(undefined, {
    ...
    entityType: 'site',
    entityId: site.id,
    eventType: resetToUnderReview ? 'site.resubmitted' : 'site.updated',
    actorType: (cognitoUser.role || 'assetmanager').toLowerCase() as any,
    actorId: effectiveUserId,
    previousState: { status: existing.status, name: existing.name },
    newState: { status: site.status, name: site.name },
  })
...
/**
 * GET /sites/v1/:siteId/activity-logs
 * Get activity logs for a specific site.
 * Only accessible by the owner (Asset Manager) or an Admin.
 */
export async function getSiteActivityLogsHandler(c: Context): Promise<Response> {
  const cognitoUser = getCognitoUser(c)
  const effectiveUserId = await getAuthenticatedUserId(cognitoUser)
  const siteId = c.req.param('siteId')
  ...
  const site = await db.site.findUnique({
    where: { id: siteId },
    select: { assetManagerId: true, deletedAt: true },
  })
  ...
  const [logs, total] = await Promise.all([
    ...
    db.auditLog.count({ where: { siteId } }),
  ])

  // Batch query user profiles for actorId to show user details
  const actorIds = Array.from(new Set(logs.map(log => log.actorId).filter(id => id !== 'system')))
  const users = actorIds.length > 0
    ? await db.user.findMany({
        ...
        select: {
          ...
          email: true,
          role: true,
          operatorProfile: { select: { fullName: true, vaId: true } },
          assetManagerProfile: { select: { fullName: true, vaId: true } },
        },
      })
    : []
  ...
  return sendPaginatedResponse(c, {
    ...
    message: 'Site activity logs retrieved successfully',
  })
}
