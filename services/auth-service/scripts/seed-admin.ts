...
async function seedAdmin() {
  ...
  if (!email || !password) {
    console.warn("⚠️ DEFAULT_ADMIN_EMAIL or DEFAULT_ADMIN_PASSWORD is not set. Skipping admin seed.");
    return;
  }
  ...
  try {
    ...
    try {
      ...
      const subAttribute = createResponse.User?.Attributes?.find((attr: any) => attr.Name === "sub");
      ...
      if (cognitoError.name === "UsernameExistsException") {
        ...
        console.error("User exists in Cognito but not in the database. Please clean up Cognito or sync manually.");
        process.exit(1);
      }
      throw cognitoError;
    }
