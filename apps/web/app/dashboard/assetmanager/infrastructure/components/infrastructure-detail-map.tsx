...
function toLatLngLike(point: unknown): LatLngLike | null {
...
function getPolygonCenter(points?: unknown[]): LatLngLike | null {
  if (!points || points.length === 0) return null

  const normalizedPoints = points
    .map(toLatLngLike)
    .filter(Boolean) as LatLngLike[]
  ...
  if (normalizedPoints.length < 3) {
    ...
    return {
      lat: sum.lat / normalizedPoints.length,
      lng: sum.lng / normalizedPoints.length,
    }
  }
  ...
  for (let index = 0; index < normalizedPoints.length; index++) {
    ...
    twiceArea += cross
    centerLat += (current.lat + next.lat) * cross
    centerLng += (current.lng + next.lng) * cross
  }
  ...
  return {
    lat: centerLat / (3 * twiceArea),
    lng: centerLng / (3 * twiceArea),
  }
}

function getSiteAnchor(site: DetailedSite): LatLngLike {
  const toalCenter =
    site.siteType !== 'emergency' && site.toalGeometryMode === 'polygon'
      ? getPolygonCenter(site.toalPolygonPoints)
      : null
  ...
  const emergencyCenter =
    site.siteType === 'emergency' || site.allowEmergencyLanding
      ? site.emergencyGeometryMode === 'polygon'
        ? getPolygonCenter(site.emergencyPolygonPoints)
        : null
      : null
...
function getSiteBounds(L: any, site: DetailedSite) {
  ...
  latLngs.push([anchor.lat, anchor.lng])
...
/**
 * Full-height map showing all assetmanager infrastructure sites.
    ...
    <div className={cn('relative h-full', className)}>
      ...
      {activeSite && (
        <div className="absolute bottom-3 left-3 z-10 pointer-events-none">
          ...
          >
            {activeSite.latitude.toFixed(6)}, {activeSite.longitude.toFixed(6)}
          </Badge>
        </div>
      )}
