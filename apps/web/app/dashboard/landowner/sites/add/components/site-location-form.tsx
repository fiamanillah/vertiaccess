...
import {
    MapPin, ArrowRight, ArrowLeft, Circle, Ruler, Pentagon, Map,
...
function circleAreaM2(r: number) { return Math.PI * r * r; }
...
function GeometrySelector({ value, color = '#5b6cf9', onChange }: GeomSelectorProps) {
    return (
        <div className="grid grid-cols-2 gap-3">
            <button
                ...
                className={cn(
                    ...
                    value === 'circle' ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/30 hover:bg-muted/20',
                )}
            >
            ...
            <button
                ...
                className={cn(
                    'flex items-center gap-3 p-3 rounded-xl border-2 text-left transition-all duration-200',
                    value === 'polygon' ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/30 hover:bg-muted/20',
                )}
            >
                <div className={cn('flex h-8 w-8 items-center justify-center rounded-full shrink-0',
...
function RadiusControl({ label, value, min = 25, max = 2000, accentColor = '#5b6cf9', areaValue, onChange }: RadiusControlProps) {
    return (
...
export function SiteLocationForm({ form, isLoading, onNext, onPrev, isLocked, globalDisabled }: SiteLocationFormProps) {
    ...
    // TOAL boundary
    const [toalMode, setToalMode] = React.useState<GeometryMode>(values.toalGeometryMode ?? 'circle');
    const [toalRadius, setToalRadius] = React.useState(values.toalRadius ?? DEFAULT_TOAL_RADIUS);
    const [toalPts, setToalPts] = React.useState<[number, number][]>(values.toalPolygonPoints ?? []);

    // Emergency boundary
    const [showEmergency, setShowEmergency] = React.useState(values.allowEmergencyLanding ?? isEmergencyPrimary);
    const [emergencyMode, setEmergencyMode] = React.useState<GeometryMode>(values.emergencyGeometryMode ?? 'circle');
    const [emergencyRadius, setEmergencyRadius] = React.useState(values.emergencyRadius ?? DEFAULT_EMERGENCY_RADIUS);
    const [emergencyPts, setEmergencyPts] = React.useState<[number, number][]>(values.emergencyPolygonPoints ?? []);
    ...
    // Computed areas
    const toalArea = toalMode === 'circle' ? fmt(circleAreaM2(toalRadius)) : fmt(polygonAreaM2(toalPts));
    const emergencyArea = emergencyMode === 'circle' ? fmt(circleAreaM2(emergencyRadius)) : fmt(polygonAreaM2(emergencyPts));
    ...
    React.useEffect(() => {
        ...
        form.setValue('toalAreaM2', toalMode === 'circle' ? circleAreaM2(toalRadius) : polygonAreaM2(toalPts));
        // Emergency
        form.setValue('allowEmergencyLanding', showEmergency);
        if (showEmergency) {
            form.setValue('emergencyGeometryMode', emergencyMode);
            form.setValue('emergencyRadius', emergencyMode === 'circle' ? emergencyRadius : undefined);
            form.setValue('emergencyPolygonPoints', emergencyMode === 'polygon' ? emergencyPts : []);
            form.setValue('emergencyAreaM2', emergencyMode === 'circle' ? circleAreaM2(emergencyRadius) : polygonAreaM2(emergencyPts));
        }
    ...
    const handleGeometryModeChange = (boundary: 'toal' | 'emergency', mode: GeometryMode) => {
        ...
        else { setEmergencyMode(mode); setEmergencyPts([]); }
    };
        ...
        <Card className="shadow-md border-border/60">
            <CardHeader className="relative overflow-hidden pb-6 border-b">
                ...
                <div className="relative z-10">
                    ...
                    <CardDescription className="mt-1">
                        Where is your site and what are its exact spatial limits?
                    </CardDescription>
                </div>
            </CardHeader>
            ...
            <CardContent className="pt-8">
                    ...
                    <div className="mb-8 p-4 rounded-xl bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/40 flex gap-3 text-amber-800 dark:text-amber-200">
                        ...
                        <div>
                            ...
                            <p className="text-xs mt-1 opacity-80">
                                For safety and compliance, location boundaries and coordinates cannot be edited after approval. Please contact support if you need to update these.
                            </p>
                        </div>
                    </div>
                )}
                <fieldset disabled={isLocked || globalDisabled} className="space-y-8">
                    ...
                    {/* ─── Address ─────────────────────────────────────── */}
                    <FieldSection title="Address" description="Exact physical location of your site">
                        <FieldGroup className="gap-6">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                ...
                                <Controller name="postcode" control={form.control}
                                    render={({ field, fieldState }) => (
                                        <Field data-invalid={fieldState.invalid}>
                    ...
                    {!isEmergencyPrimary && (
                        <>
                            <FieldSection
                                title="TOAL Boundary"
                                description="Define the Take-off & Landing operational boundary"
                            >
                                <div className="space-y-4">
                            ...
                            {/* ─── Emergency Toggle ────────────────────── */}
                            <FieldSection title="Emergency Landings" description="Allow emergency & recovery landings at this site">
                                <button
                                    type="button"
                                    onClick={() => {
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        <div className={cn(
                                            ...
                                            showEmergency ? 'bg-amber-100 dark:bg-amber-900/40' : 'bg-muted'
                                        )}>
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-muted-foreground mt-0.5">
                                                {showEmergency ? 'Emergency landing boundary is active' : 'Tap to enable and define the recovery area'}
                                            </p>
                                        </div>
                                    </div>
                                    {showEmergency
                    ...
                    {showEmergency && (
                        <>
                            <FieldSection
                                ...
                                description={isEmergencyPrimary
                                ...
                                <div className="space-y-4">
                                    ...
                                    {emergencyMode === 'circle' && (
                                        <RadiusControl
                                            ...
                                            onChange={(v) => setEmergencyRadius(Math.max(toalRadius + 25, v))}
                                        />
                                    )}
                                    ...
                                    {emergencyMode === 'polygon' && (
                                        <>
                                            <div className="rounded-xl bg-amber-50 dark:bg-amber-950/20 border border-amber-300/40 px-4 py-3">
                                                <p className="text-xs text-muted-foreground leading-relaxed">
                                                    <span className="font-semibold text-foreground">Emergency drawing mode.</span>{' '}
                                                    Switch to the <span className="font-semibold text-amber-500">Emergency</span> tab on the map, then click points to draw the recovery boundary.
                                                </p>
                                            </div>
                    ...
                    <FieldSection
                        title="Map Interface"
                        description={isLocked ? "Current site boundaries" : "Use the search bar to find your site, then click to place the boundary"}
                    >
                        {isLocked ? (
                    ...
                    <div className="flex items-center justify-between pt-4 border-t">
                        ...
                        <Button
                            type="button" onClick={onNext} disabled={isLoading}
