...
function InfoRow({ label, value, fullWidth }: { label: string; value: React.ReactNode; fullWidth?: boolean }) {
	return (
		<div className={cn('flex flex-col gap-1 py-2.5 border-b border-border/20 last:border-0', fullWidth && 'col-span-2')}>
			...
			<div className="text-sm font-semibold text-foreground/90 leading-relaxed min-h-[20px] flex items-center">
				{value || <span className="text-muted-foreground font-normal italic text-xs">Not provided</span>}
			</div>
		</div>
	);
}
...
export function SiteReviewForm({ form, isLoading, onSubmit, onPrev, onJumpToStep, editModeStatus }: SiteReviewFormProps) {
		...
		<Card className="shadow-md border-border/60">
			...
			<CardContent className="pt-8">
				<div className="space-y-8">
					...
					<ReviewSection title="Site Identity" icon={Building2} onEdit={() => onJumpToStep(1)} step={1}>
						<div className="grid grid-cols-2 gap-x-6">
							...
							{values.description && (
								<InfoRow label="Description" value={values.description} fullWidth />
							)}
						</div>
						{/* Photos strip */}
						<div className="pt-3">
					...
					<ReviewSection title="Location & Boundaries" icon={MapPin} onEdit={() => onJumpToStep(2)} step={2}>
						<div className="grid grid-cols-2 gap-x-6">
							...
							<InfoRow
								...
								value={
									values.allowEmergencyLanding ? (
										<Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-100 border-none">Enabled — {values.emergencyRadius}m</Badge>
									) : (
						...
						<div className="mt-4 rounded-xl overflow-hidden border border-border/40">
							<PreviewMap
								center={{ lat: values.latitude || 51.505, lng: values.longitude || -0.09 }}
								...
								initialEmergencyPolygonPoints={values.emergencyPolygonPoints || []}
							/>
						</div>
					</ReviewSection>

					{/* ─── Step 3: Operational Policy ──────────────────────── */}
					<ReviewSection title="Operational Policy" icon={CalendarIcon} onEdit={() => onJumpToStep(3)} step={3}>
						<div className="grid grid-cols-2 gap-x-6">
							<InfoRow
								label="Activation Window"
								value={
									values.isPermanentActivation
										? <Badge className="bg-sky-100 text-sky-700 hover:bg-sky-100 border-none">Permanent</Badge>
							...
							<InfoRow
								...
								value={values.bookingApprovalModel === 'auto' ? 'Auto-Approval (Instant)' : 'Manual Review'}
							/>
						</div>
						<div className="pt-3 border-t border-border/30 mt-3">
							<span className="text-[9px] uppercase tracking-widest text-muted-foreground font-bold">Policy Documents</span>
							<DocumentList urls={values.policyDocuments} label="Document" />
						</div>
					</ReviewSection>

					{/* ─── Step 4: Commercial Setup ─────────────────────────── */}
					<ReviewSection title="Pricing & Earnings" icon={Banknote} onEdit={() => onJumpToStep(4)} step={4}>
						<div className="grid grid-cols-2 gap-x-6">
							<InfoRow label="TOAL Access Fee" value={`£${values.toalFee?.toFixed(2) ?? '0.00'}`} />
							<InfoRow label="Emergency Access Fee" value={`£${values.emergencyFee?.toFixed(2) ?? '0.00'}`} />
							<InfoRow
					...
					<ReviewSection title="Proof of Authority" icon={Gavel} onEdit={() => onJumpToStep(5)} step={5}>
						<div className="grid grid-cols-2 gap-x-6">
							<InfoRow
								label="Legal Declaration"
								value={
									values.legalDeclaration
										? <Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-100 border-none uppercase text-[10px] px-2 h-5">Confirmed</Badge>
										: <Badge variant="destructive" className="uppercase text-[10px] px-2 h-5">Not Confirmed</Badge>
								}
							/>
						</div>
						<div className="pt-3 border-t border-border/30 mt-3">
							<span className="text-[9px] uppercase tracking-widest text-muted-foreground font-bold">Ownership Documents</span>
							<DocumentList urls={values.ownershipDocuments} label="Ownership Proof" />
						</div>
					</ReviewSection>
					...
					<div className="space-y-4 pt-4 border-t border-border/60">
						<div className="flex items-center gap-2.5">
							...
							<div className="flex flex-col">
								<h3 className="font-bold text-sm tracking-tight">Landowner Declaration</h3>
						...
						<div className="bg-muted/30 border border-border/40 rounded-xl p-4 space-y-4">
							<div className="flex items-start gap-2 text-xs text-muted-foreground">
								...
								<p className="leading-relaxed">
									The following declarations are <strong>legally binding</strong>. Please read carefully before checking each box. Submitting with false declarations may result in legal action.
								</p>
							</div>
							...
							<div
								className={cn(
									...
									termsAccepted ? 'bg-primary/5 border-primary/20 shadow-sm' : 'bg-background border-border/50'
								)}
							>
								<Checkbox
									...
									className="mt-0.5"
								/>
								<label htmlFor="terms-declaration" className="text-xs leading-relaxed cursor-pointer text-foreground/90">
									...
									, and that all information provided in this application is accurate and complete.
								</label>
							</div>
							...
							<div
								className={cn(
									'flex items-start gap-3 p-3 rounded-lg border transition-colors',
									safetyAccepted ? 'bg-primary/5 border-primary/20 shadow-sm' : 'bg-background border-border/50'
								)}
							>
								<Checkbox
									...
									className="mt-0.5"
								/>
								<label htmlFor="safety-declaration" className="text-xs leading-relaxed cursor-pointer text-foreground/90">
									I accept full responsibility for ensuring the safety of all drone operations on my land, including compliance with{' '}
									<span className="font-semibold">UK CAA regulations</span> and all applicable local authority requirements. I understand that VertiAccess acts solely as an intermediary platform and holds no liability for on-site incidents.
								</label>
							</div>
						...
						>
							{isLoading ? (
								<span className="flex items-center gap-2">
									...
									{editModeStatus === 'active' ? 'Saving Changes...' : 'Submitting...'}
								</span>
							) : (
								<>
									<CheckCircle2 className="h-4 w-4" />
									{editModeStatus === 'active' ? 'Save Changes' : editModeStatus === 'rejected' ? 'Resubmit Site' : 'Submit Application'}
								</>
							)}
						</Button>
					</div>
