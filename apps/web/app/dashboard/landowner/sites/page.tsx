...
const mockSites: DetailedSite[] = [
	...
	},
	{
		...
		toalPolygonPoints: [[53.477, -2.253], [53.478, -2.253], [53.478, -2.252], [53.477, -2.252]],
		...
		isPermanentActivation: true,
		bookingApprovalModel: 'manual',
		policyDocuments: ['https://example.com/mcr-safety.pdf', 'https://example.com/mcr-ops.pdf'],
		toalFee: 85.00,
		emergencyFee: 0.00,
		status: 'active',
...
export default function MySitesPage() {
	...
	const [selectedSite, setSelectedSite] = React.useState<DetailedSite | null>(null);
	...
	const columns = React.useMemo<ColumnDef<DetailedSite>[]>(() => [
		{
			...
			cell: ({ row }) => (
				<div className="flex flex-col gap-1 py-1">
					<span className="font-bold text-sm text-foreground tracking-tight">{row.original.name}</span>
					<div className="flex items-center gap-1.5">
						<Badge variant="secondary" className="text-[9px] uppercase tracking-widest h-4 px-1 font-bold bg-muted/50 border-none">
			...
			cell: ({ row }) => (
				<div className="flex flex-col gap-0.5">
					<div className="flex items-center gap-1.5 text-xs text-muted-foreground font-medium">
						...
						<span className="truncate max-w-[220px]">{row.original.address}</span>
					</div>
					<span className="text-[10px] text-muted-foreground/70 font-mono ml-4.5">{row.original.postcode}</span>
				</div>
			)
		},
		{
				...
				return (
					<Badge
						...
						className={cn(
							...
							isToal ? "bg-indigo-50 text-indigo-700 dark:bg-indigo-950/30" : "bg-amber-50 text-amber-700 dark:bg-amber-950/30"
						)}
					>
		...
		},
		{
			...
			cell: ({ row }) => (
				<div className="flex items-baseline gap-0.5 font-mono">
					...
					<span className="text-sm font-bold text-foreground">{row.original.toalFee.toFixed(2)}</span>
				</div>
			)
		},
		{
			...
			<div className="grid gap-4 md:grid-cols-3">
				<Card className="bg-primary/5 border-primary/10 shadow-sm overflow-hidden relative group">
					...
					<CardHeader className="pb-2">
						<CardDescription className="text-[10px] uppercase tracking-widest font-bold text-primary/70">Total Registered Sites</CardDescription>
						<CardTitle className="text-3xl font-bold tracking-tight">03</CardTitle>
					</CardHeader>
					<CardContent>
						<div className="text-[10px] text-muted-foreground font-medium flex items-center gap-1">
							<CheckCircle2 className="h-3 w-3 text-emerald-500" />
							2 sites are currently active
						</div>
					</CardContent>
				</Card>
				...
				<Card className="shadow-sm overflow-hidden relative group">
					<CardHeader className="pb-2">
						<CardDescription className="text-[10px] uppercase tracking-widest font-bold text-muted-foreground">Pending Approval</CardDescription>
						<CardTitle className="text-3xl font-bold tracking-tight text-foreground/80">01</CardTitle>
					</CardHeader>
					<CardContent>
						<div className="text-[10px] text-muted-foreground font-medium flex items-center gap-1">
				...
				<Card className="shadow-sm overflow-hidden relative group">
					<CardHeader className="pb-2">
						<CardDescription className="text-[10px] uppercase tracking-widest font-bold text-muted-foreground">Total Earnings</CardDescription>
						<CardTitle className="text-3xl font-bold tracking-tight text-foreground/80">£1,240.50</CardTitle>
					</CardHeader>
					<CardContent>
						<div className="text-[10px] text-muted-foreground font-medium flex items-center gap-1">
							<Badge className="bg-emerald-100 text-emerald-700 border-none text-[8px] h-4 px-1.5 font-bold">+12%</Badge>
				...
				<CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-4 sm:space-y-0 pb-6 border-b">
					<div className="space-y-1">
						<div className="flex items-center gap-2 mb-1">
							...
							<CardTitle className="text-xl font-bold tracking-tight">Site Management</CardTitle>
						</div>
						<CardDescription className="text-sm">
							Monitor and manage your takeoff and landing locations across the network.
						</CardDescription>
					</div>
					<Button asChild className="font-bold shadow-md shadow-primary/20 hover:shadow-lg transition-all w-full sm:w-auto">
