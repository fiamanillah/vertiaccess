    ...
    const columns: ColumnDef<Booking>[] = React.useMemo(() => [
            ...
            cell: ({ row }) => (
                <div className="flex flex-col gap-1 py-1">
                    <div className="flex items-center gap-2">
                        ...
                        <Badge variant="outline" className="text-[9px] uppercase tracking-widest h-4 px-1 font-bold bg-muted/50 border-none text-muted-foreground">
                            {row.original.operatorOrganisation || 'Independent'}
                        </Badge>
                    </div>
                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                        <span className="truncate max-w-[150px] italic">"{row.original.missionIntent}"</span>
                    </div>
                </div>
            )
        },
                ...
                return (
                    <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-1.5 text-xs text-muted-foreground font-medium">
                            ...
                            <span className="truncate max-w-[180px]">{row.original.siteName}</span>
                        </div>
                        <div className="flex items-center gap-2">
                            {isEmergency ? (
                        ...
                        {isCancelled ? (
                            ...
                            <>
                                <div className="flex items-baseline gap-0.5 font-mono">
                                    ...
                                    <span className="text-sm font-bold text-foreground">{booking.toalCost?.toFixed(2)}</span>
                                </div>
        ...
        },
        {
            ...
            cell: ({ row }) => {
                ...
                if (status === 'APPROVED' && isEmergency && isPast && booking.paymentStatus === 'pending') {
                    return (
                        <Tooltip>
                            <TooltipTrigger asChild>
                            ...
                            <TooltipContent className="max-w-[200px]">
                                The flight window has closed. The operator must confirm if they utilized your site before payment is released.
                            </TooltipContent>
                        </Tooltip>
                    );
                }
                    ...
                    <Badge
                        className={cn(
                            ...
                            status === 'PENDING' ? "bg-amber-100 text-amber-700 hover:bg-amber-100" :
                                status === 'APPROVED' ? "bg-emerald-100 text-emerald-700 hover:bg-emerald-100" :
