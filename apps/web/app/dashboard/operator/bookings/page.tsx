...
const mockBookings: Booking[] = [
    ...
    },
    {
        ...
        cancelledAt: null,
        adminNote: 'Message from Landowner: Please redraw your flight path, it overlaps with the cattle field on the east side.',
    }
];
...
export default function OperatorBookingsPage() {
    ...
    const upcomingBookings = bookings.filter(b => b.status === 'APPROVED' && new Date(b.endTime) > new Date());
    ...
    const unresolvedEmergency = bookings.find(b => 
    ...
    const handleEmergencyResolve = (booking: Booking, used: boolean) => {
        setBookings(prev => prev.map(b => 
            b.id === booking.id ? { ...b, paymentStatus: used ? 'paid' : 'refunded' } : b
        ));
        toast.success(used ? "Emergency usage confirmed. Processing payment." : "No usage confirmed. Reservation released.");
    };
    ...
    const handleDownloadCertificate = (booking: Booking) => {
        toast.success(`Downloading Consent Certificate for ${booking.bookingReference}`);
    };
    ...
    return (
        <div className="flex flex-1 flex-col gap-6 p-4 md:p-8 max-w-6xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-black tracking-tight uppercase">Flight Itinerary</h1>
                    <p className="text-muted-foreground text-[10px] font-black uppercase tracking-[0.2em] mt-1">Manage your missions & authorizations</p>
                </div>
            ...
            <Tabs defaultValue="upcoming" className="w-full">
                <TabsList className="bg-muted/30 p-1 mb-8 border border-border/40 inline-flex w-auto rounded-xl">
                    <TabsTrigger value="upcoming" className="px-6 font-black text-[10px] uppercase tracking-widest data-[state=active]:bg-background data-[state=active]:shadow-sm rounded-lg">
                        Upcoming Flights
                    ...
                    <TabsTrigger value="pending" className="px-6 font-black text-[10px] uppercase tracking-widest data-[state=active]:bg-background data-[state=active]:shadow-sm rounded-lg">
                        Pending Requests
                    ...
                    <TabsTrigger value="past" className="px-6 font-black text-[10px] uppercase tracking-widest data-[state=active]:bg-background data-[state=active]:shadow-sm rounded-lg">
                        Logbook
                    </TabsTrigger>
                </TabsList>
