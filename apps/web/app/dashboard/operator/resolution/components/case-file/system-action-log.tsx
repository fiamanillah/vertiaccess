...
import { Settings, ShieldAlert, CreditCard, FileSearch, ArrowRight } from 'lucide-react';
...
export function SystemActionLog({ log }: SystemActionLogProps) {
    const getIcon = () => {
        if (log.content.includes('Refund') || log.content.includes('payment')) return <CreditCard className="h-3 w-3" />;
        if (log.content.includes('requested') || log.content.includes('logs')) return <FileSearch className="h-3 w-3" />;
        if (log.content.includes('sanction') || log.content.includes('suspended')) return <ShieldAlert className="h-3 w-3" />;
