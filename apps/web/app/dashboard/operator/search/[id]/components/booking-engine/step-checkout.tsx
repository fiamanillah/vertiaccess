...
export function StepCheckout({
    ...
    const platformFee = operationType === 'toal' ? currentFee * 0.05 : 0;
                    ...
                    {operationType === 'toal' ? (
                        <>
                            ...
                            <div className="flex justify-between items-center pt-1">
                                ...
                                <span className="text-2xl font-black text-primary tracking-tighter">£{totalDue.toFixed(2)}</span>
                            </div>
                            <div className="flex items-start gap-2 bg-primary/5 p-3 rounded-lg border border-primary/10">
                                ...
                                <p className="text-[10px] text-primary/80 font-medium leading-relaxed">
                                    Your card will be charged automatically on the booking start date.
                                </p>
                            </div>
                        </>
                    ) : (
                        <>
                            ...
                            <div className="flex justify-between items-center text-sm">
                                <span className="text-amber-700 font-bold uppercase tracking-wider text-[10px]">Emergency Fee (if used)</span>
                                <span className="font-black text-amber-700">£{currentFee.toFixed(2)}</span>
                            </div>
                            ...
                            <div className="flex items-start gap-2 bg-amber-500/5 p-3 rounded-lg border border-amber-500/10">
                                ...
                                <p className="text-[10px] text-amber-700/80 font-medium leading-relaxed">
                                    <strong>£0.00 is charged today.</strong> £{currentFee.toFixed(2)} will only be
                                    charged to your card ending {defaultCard?.last4 ?? '••••'} if you confirm site
                            ...
                            >
                                ...
                                <span className="text-[10px] text-amber-800 font-medium leading-relaxed">
                                    ...
                                    landing, or if an admin dispute finds that I utilised the site.
                                </span>
                            </label>
                        </>
                    )}
            ...
            <div className="p-4 rounded-2xl border border-border bg-muted/20 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-3">
                    ...
                    <div className="space-y-0.5">
                        ...
                        {defaultCard ? (
                            <>
                                <p className="text-xs font-bold tracking-widest">•••• {defaultCard.last4}</p>
                                <p className="text-[9px] text-muted-foreground capitalize">
                                    {defaultCard.brand} — {defaultCard.expiryMonth}/{defaultCard.expiryYear}
                                </p>
                            </>
                        ) : (
                            <p className="text-xs font-bold text-destructive">No card on file</p>
                        )}
                    </div>
                </div>
                <Button variant="ghost" size="xs" className="text-[10px] font-black text-primary hover:bg-primary/5 uppercase tracking-widest">
                    CHANGE
                </Button>
            </div>
        </div>
