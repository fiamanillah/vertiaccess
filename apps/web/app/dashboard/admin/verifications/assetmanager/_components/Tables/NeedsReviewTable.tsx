  ...
  const columns: ColumnDef<VerificationRequest>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        let badgeClass = "bg-amber-100 text-amber-700 border-none text-xs font-semibold px-2.5 py-0.5 h-6 capitalize"
        if (status === 'VERIFIED') {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Button size="sm" variant="outline" className="gap-2 h-8 px-3" asChild>
          <Link
            href={`/dashboard/admin/verifications/assetmanager/review/${row.original.id}`}
          >
