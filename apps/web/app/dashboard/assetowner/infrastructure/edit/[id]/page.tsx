...
export default function EditInfrastructureAssetPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    function mapBackendSiteToFormValues(s: any): FormValues {
        ...
        return {
            ...
            ownershipDocuments: mappedOwnershipDocuments, legalDeclaration: s.status === 'ACTIVE' ? true : !!s.acceptedAssetOwnerDeclaration,
        };
    }
    ...
    async function onSubmit(data: FormValues) {
        ...
        const payload: any = {
            ...
            authorizedToGrantAccess: true, acceptedAssetOwnerDeclaration: !!data.legalDeclaration,
        };
            ...
            <div className="flex-1 lg:w-[45%] lg:flex-none border-r flex flex-col min-w-0">
                <div className="px-6 py-5 border-b bg-muted sticky top-16 z-40 flex flex-col gap-4">
                    <div className="flex items-center gap-3">
                        <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-foreground shrink-0" onClick={() => { if (currentStep > 1) { prevStep() } else { router.push('/dashboard/assetowner/infrastructure') } }}>
                ...
                <div className="flex-1 p-4 sm:p-6 pb-24 lg:pb-12 overflow-y-auto">
                    ...
                    {currentStep === 1 && (
                        <SiteInformationForm form={form} isLoading={isLoading} onNext={nextStep} onCancel={() => router.push('/dashboard/assetowner/infrastructure')} isIdentityLocked={siteStatus === 'active'} globalDisabled={siteStatus === 'pending'} />
                    )}
                    {currentStep === 2 && (
