...
export function apiBookingToFrontend(b: ApiBooking): BookingRequest {
  return {
    ...
    isPAYG: b.isPayg,
    certificateVaId: b.certificateVaId || undefined,
 ...
 */
export async function apiFetchLandownerBookings(
  idToken: string,
  params?: {
    page?: number
    limit?: number
  ...
  const fetchPage = async (page: number) => {
    const url = new URL(`${getApiBaseUrl()}/bookings/v1/landowner`)
    url.searchParams.set('page', String(page))
    url.searchParams.set('limit', String(params?.limit ?? 50))

    if (params?.status) url.searchParams.set('status', params.status)
    if (params?.useCategory)
      url.searchParams.set('useCategory', params.useCategory)
    ...
    const json = await res.json()
    if (!res.ok)
      throw new Error(json?.message || 'Failed to fetch landowner bookings')
    return json as {
      data: ApiBooking[]
  ...
  if (typeof params?.page === 'number') {
    const pageResult = await fetchPage(params.page)
  ...
  while (true) {
    ...
    if (!pageResult.meta?.pagination?.hasNext) {
      break
    }
 ...
 */
export async function apiFetchSiteBookings(
  idToken: string,
  siteId: string,
): Promise<ApiBooking[]> {
...
export async function apiUpdateBookingStatus(
  ...
  const res = await fetch(
    `${getApiBaseUrl()}/bookings/v1/${bookingId}/status`,
    {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
...
export async function apiCreateBookingPaymentIntent(
  ...
  const res = await fetch(
    `${getApiBaseUrl()}/billing/v1/booking-payment-intent`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
export async function apiGetBookingCertificate(
  ...
  const res = await fetch(
    `${getApiBaseUrl()}/bookings/v1/${bookingDbId}/certificate`,
    {
      headers: { Authorization: `Bearer ${idToken}` },
    },
  )
...
export async function apiPayLandownerBooking(
  ...
  const res = await fetch(
    `${getApiBaseUrl()}/billing/v1/bookings/${bookingDbId}/pay`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
export async function apiConfirmEmergencyUsage(
  ...
  const res = await fetch(
    `${getApiBaseUrl()}/bookings/v1/${bookingDbId}/emergency-usage`,
    {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
  ...
  const json = await res.json()
  if (!res.ok)
    throw new Error(json?.message || 'Failed to confirm emergency usage')
  return json?.data as ApiBooking
}
...
export async function apiFetchPublicSiteAvailability(
  ...
  const res = await fetch(
    `${getApiBaseUrl()}/bookings/v1/availability/${siteId}${suffix}`,
  )
  const json = await res.json()
