        ...
        <div className="space-y-6">
          ...
          <div className="bg-blue-50/50 p-6 rounded-2xl border border-blue-100/50">
            ...
            <p className="text-xs text-blue-800/70 leading-relaxed">
              Market-competitive rates for {siteCategory === 'private_land' ? 'Private Land' : 'Private Rooftop'} sites. You retain full control over pricing.
            </p>
          </div>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <p className="text-xs text-slate-500">Amount paid by the operator per approved operation.</p>
            </div>

            {siteType === 'toal' && emergencyRecoveryEnabled && (
              <div className="space-y-2 pt-4 border-t border-slate-100">
                <div className="flex items-center justify-between">
                  <label className="text-sm font-bold text-slate-700">Emergency Access Fee *</label>
                </div>
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-bold">£</span>
                  <input
                ...
                <p className="text-xs text-slate-500">Premium fee for emergency contingency recovery landings.</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="space-y-6">
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <span className="text-sm text-slate-500 font-medium">Access Fee (Base)</span>
                <span className="font-bold text-slate-900 text-lg">£{Number(accessFee || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
              </div>
              
              {siteType === 'toal' && emergencyRecoveryEnabled && (
                <div className="flex justify-between items-center">
                  <span className="text-sm text-slate-500 font-medium">Emergency Fee</span>
                  <span className="font-bold text-slate-900 text-lg">£{Number(clzAccessFee || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                </div>
              )}
              ...
              <div className="flex justify-between items-center pt-2">
                ...
                <div className="text-right">
                  <span className="text-4xl font-bold text-emerald-500 tracking-tight">
                    £{Number(accessFee || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </span>
                </div>
              </div>
              {siteType === 'toal' && emergencyRecoveryEnabled && (
                <p className="text-xs text-slate-400 italic text-right -mt-1 font-medium">
                  + £{Number(clzAccessFee || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} if emergency access is utilised
                </p>
              )}
            </div>
            ...
            <div className="pt-6 border-t border-slate-200">
              <div className="flex justify-between items-center">
                <span className="text-sm text-slate-500 font-medium">Operator Pays</span>
                <span className="text-lg font-bold text-slate-900">£{Number(accessFee || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
              </div>
            </div>
            ...
            <div className="bg-white/60 p-5 rounded-2xl border border-slate-200/60 mt-4">
              <p className="text-xs text-slate-400 leading-relaxed italic font-medium">
                Transparency builds trust. Even if platform fees are £0.00, we show them to maintain clear financial records.
              </p>
            </div>
          </div>
        </div>
