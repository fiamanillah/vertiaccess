...
export function SitesDiscoveryMap({ sites, onSiteClick }: SitesDiscoveryMapProps) {
    ...
    useEffect(() => {
        ...
        // Ensure Leaflet measures container correctly after render
        setTimeout(() => {
            try {
                map.invalidateSize();
            } catch (err) {
    ...
    useEffect(() => {
        ...
        filteredSites.forEach(site => {
            ...
            if (site.geometry && site.geometry.type === 'circle' && site.geometry.center) {
                ...
                site.geometry.points &&
                site.geometry.points.length > 0
            ) {
            ...
            // Fallback to CLZ geometry if primary is missing
                ...
                site.clzGeometry &&
                site.clzGeometry.type === 'circle' &&
                site.clzGeometry.center
            ) {
                center = site.clzGeometry.center;
            } else if (
                site.clzGeometry &&
                site.clzGeometry.type === 'polygon' &&
                site.clzGeometry.points &&
                site.clzGeometry.points.length > 0
            ) {
            ...
            if (site.clzEnabled && site.clzGeometry) {
                if (
                    site.clzGeometry.type === 'circle' &&
                    site.clzGeometry.center &&
                    site.clzGeometry.radius
                ) {
                    const clzCircle = L.circle(
                        [site.clzGeometry.center.lat, site.clzGeometry.center.lng],
                        {
                    ...
                    site.clzGeometry.type === 'polygon' &&
                    site.clzGeometry.points &&
                    site.clzGeometry.points.length >= 3
                ) {
      ...
      `);
        ...
        // After adding markers, fit map to their bounds so they are visible
        try {
            const latlngs: L.LatLngExpression[] = markersRef.current.map(m => m.getLatLng());
            if (latlngs.length > 0) {
    ...
    const handleSearch = (e: React.FormEvent) => {
        ...
        if (searchQuery && mapRef.current && sites.length > 0) {
            ...
            if (matchingSite) {
                ...
                if (
                    matchingSite.geometry &&
                    matchingSite.geometry.type === 'circle' &&
                    matchingSite.geometry.center
                ) {
                    center = matchingSite.geometry.center;
                } else if (
                    matchingSite.geometry &&
                    matchingSite.geometry.type === 'polygon' &&
                    matchingSite.geometry.points &&
                    matchingSite.geometry.points.length > 0
                ) {
                    ...
                    matchingSite.clzGeometry &&
                    matchingSite.clzGeometry.type === 'circle' &&
                    matchingSite.clzGeometry.center
                ) {
                    center = matchingSite.clzGeometry.center;
                } else if (
                    matchingSite.clzGeometry &&
                    matchingSite.clzGeometry.type === 'polygon' &&
                    matchingSite.clzGeometry.points &&
                    matchingSite.clzGeometry.points.length > 0
                ) {
