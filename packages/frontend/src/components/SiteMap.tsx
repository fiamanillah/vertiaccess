...
export function SiteMap({
  ...
  const [selectedDrawingMode, setSelectedDrawingMode] = useState<
    "toal" | "emergency" | null
  >(null);
  ...
  const [activeCircleMode, setActiveCircleMode] = useState<
    "toal" | "emergency" | null
  >(null);
    ...
    const timer = setTimeout(
      () => {
        ...
        if (readonly && (toalLayerRef.current || clzLayerRef.current)) {
          const bounds =
            clzLayerRef.current?.getBounds() ||
            toalLayerRef.current?.getBounds();
  ...
  const currentPoints =
    drawingMode === "emergency" ? clzPolygonPoints : polygonPoints;
