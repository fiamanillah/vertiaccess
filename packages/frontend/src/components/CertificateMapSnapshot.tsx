...
export function CertificateMapSnapshot({ toalGeometry, clzGeometry, siteName }: CertificateMapSnapshotProps) {
  ...
  useEffect(() => {
    const initMap = async () => {
      ...
      } else if (toalGeometry.type === 'polygon' && toalGeometry.points && toalGeometry.points.length > 0) {
      ...
      if (clzGeometry) {
        if (clzGeometry.type === 'circle' && clzGeometry.center && clzGeometry.radius) {
          ...
          const isDonut = toalGeometry.type === 'circle' && 
                         toalGeometry.center && 
        ...
        } else if (clzGeometry.type === 'polygon' && clzGeometry.points && clzGeometry.points.length >= 3) {
          L.polygon(
      ...
      if (toalGeometry.type === 'circle' && toalGeometry.center && toalGeometry.radius) {
        ...
        L.marker([toalGeometry.center.lat, toalGeometry.center.lng], { icon: divIcon }).addTo(map);
      } else if (toalGeometry.type === 'polygon' && toalGeometry.points && toalGeometry.points.length >= 3) {
          ...
          <div className="mt-2 space-y-1">
            ...
            {clzGeometry && (
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-gray-700">Emergency & Recovery Zone</span>
              </div>
            )}
          </div>
        </div>
